...
if (!isset($_SESSION['email_event']))
   $_SESSION['email_event'] = false;
$error_flag = false;
// initialize curl struct
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL,            "https://webhook.logentries.com/noformat/logs/9a88c25e-e695-4c81-b0ca-753c1d0a75df" );
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1 );
curl_setopt($ch, CURLOPT_POST,           1 );
curl_setopt($ch, CURLOPT_HTTPHEADER,     array('Content-Type: text/plain')); 

// we need to get process number from DB and put into URL - later
$packageId=$_REQUEST['patientEpisodeId'];
//$packageId="26_ug07Proms_patientEpisode";
$loc=strpos($packageId,'_');
$loc2=strrpos($packageId,'_');
$packageId=substr($packageId,$loc+1,$loc2-$loc-1);
//Add code here*********
// echo "PackageID:  $packageId<br />";
if ($packageId=="ug07Proms")
   $tableName="app_fd_pro_patientEpisodes";
else
   $tableName="app_fd_".$packageId."_patientEpisodes";
//echo $packageId;
//exit();
...
if (isset($_POST['submit']) && !empty($_POST['submit'])) {
   if (isset($_REQUEST['patientEpisodeId']) && !empty($_REQUEST['patientEpisodeId'])) {
      ...
      if (!$_SESSION['email_event'])
      {
         //event log - curl for "Email clicked"
         $curl_fields = 'EIDO - {"system": "UG07 PROMS","subsystem":"Validation PHP", "Event": "EMail clicked", "episodeId": '.$id.'}';
         curl_setopt($ch, CURLOPT_POSTFIELDS, $curl_fields ); 
         $result=curl_exec ($ch);         
         $_SESSION['email_event'] = true;
      }

      $db->where('id', $id);
// echo "Table:  $tableName<br />";
      $patientCheck = $db->getOne($tableName);
      ...
      if ($error_flag)
      {
         // event log - curl for "Patient validation error"
         $curl_fields = 'EIDO - {"system": "UG07 PROMS","subsystem":"Validation PHP", "Event": "Patient validation error", "episodeId": '.$id.'}';
         curl_setopt($ch, CURLOPT_POSTFIELDS, $curl_fields ); 
         $result=curl_exec ($ch);         
      }
      ...
      $patientDetail = $db->getOne('app_fd_pro_patientEpisodes');
...
//Change the server URL for live vs. dev
            $URL = "http://clearos7.cyberacc.net:8080/jw/web/json/workflow/process/list?packageId=".$packageId;
...
//Change the server URL for live vs. dev
            if (isset($process_id)) {
               $URL = "http://clearos7.cyberacc.net:8080/jw/web/json/workflow/process/start/" . $process_id;
...
// echo "Session: $current_session<br />";
         if ($current_session == "1")
            ...
            $sid_param = $patientDetail['c_session6Survey1'] . "%3B" . $patientDetail['c_session6Survey2'] . "%3B" . $patientDetail['c_session6Survey3'] . "%3B" . $patientDetail['c_session6Survey4'] . "%3B" . $patientDetail['c_session6Survey5'];
         ...
         logMsg( "Validation: goto_url: $goto_url", "wel.log");

         // event log - curl for "Patient validated"
         $curl_fields = 'EIDO - {"system": "UG07 PROMS","subsystem":"Validation PHP", "Event": "Patient validated", "episodeId": '.$id.'}';
         curl_setopt($ch, CURLOPT_POSTFIELDS, $curl_fields ); 
         $result=curl_exec ($ch);         
